...
####Run 1: Totally Killed it####
setwd("C:/Users/Kayla/SkyDrive/PhDzNuts/FirstYearProject/Year1")
...
####Run it year by year to see if the relationship still holds#### 
#After running it for each year, it looks like there is not a relationship in the first year,
#but there is a strong relationship for the last two years
setwd("C:/Users/Kayla/SkyDrive/PhDzNuts/FirstYearProject/Year1")
...
year1<-data.frame(
  Date=seq(as.Date("2014-01-01"), as.Date("2014-12-31"),"days"))
year2<-data.frame(
  Date=seq(as.Date("2015-01-01"), as.Date("2015-12-31"),"days"))
year3<-data.frame(
  Date=seq(as.Date("2016-01-01"), as.Date("2016-10-29"),"days"))

#cross reference the dates and fill in data from original data set
year1<-merge(combodata,year1)

year2<-merge(combodata,year2)

year3<-merge(combodata,year3)

plot1<-plot(year1$Date, year1$RFHarvest, type="l",col="red1", ylab="Fish Harvest", xlab="Date", xlim=c(as.Date("2014-03-01"),as.Date("2016-11-01")), ylim=c(0,1600))
lines(year1$Date,year1$KBHarvest,col="blue")
lines(year2$Date,year2$RFHarvest, col="red")
lines(year2$Date,year2$KBHarvest, col="blue")
lines(year3$Date,year3$RFHarvest, col= "red")
lines(year3$Date,year3$KBHarvest, col= "blue")
legend("topright", legend = c("Kelp Bass Harvest", "Rockfish Harvest"), 
       col = c("blue", "red"), lwd = 1)

library(rEDM)


#### Time Series Analysis (Year1)####
...
# use smap to identify nonlinearity
smap_output <- s_map(ts, lib, pred, E = c(10))

# plot s-map output
par(mfrow = c(1,1))
plot(smap_output$theta[smap_output$E == 10], smap_output$rho[smap_output$E == 10], type = "l",xlab = "Nonlinearity (theta)", 
     ylab = "Forecast Skill (rho)")
...
KB_xmap_RF <- ccm(year1, lib, pred, E = 10, lib_sizes = seq(50, 300, by = 50), lib_column = "KBHarvest", target_column = "RFHarvest", num_samples = 100)
RF_xmap_KB <- ccm(year1, lib, pred, E = 10, lib_sizes = seq(50, 300, by = 50), lib_column = "RFHarvest", target_column = "KBHarvest", num_samples = 100)

#get single rho value for each cross map
KB_xmap_RF1 <- ccm(year1, lib, pred, E = 10, lib_sizes = length(ts), lib_column = "KBHarvest", target_column = "RFHarvest",  random_libs = FALSE)
RF_xmap_KB1 <- ccm(year1, lib, pred, E = 10, lib_sizes = length(ts), lib_column = "RFHarvest", target_column = "KBHarvest",  random_libs = FALSE)
...
plot(xmap_means[[1]]$lib_size, xmap_means[[1]]$rho, type = "l", col = "red", ylim = plot_limits, ylab="rho", xlab="Library Size")
lines(xmap_means[[2]]$lib_size, xmap_means[[2]]$rho, col = "blue")
legend("bottomright", legend = c("KB xmap RF", "RF xmap KB"), 
       col = c("red", "blue"), lwd = 1)


##Correlation Analysis (hoping for negative correlation)##
correlation1<-cor(year1$KBHarvest,year1$RFHarvest)
correlation1


#### Time Sereies Analysis (Year2)####
...
KB_xmap_RF <- ccm(year2, lib, pred, E = 10, lib_sizes = seq(50, 300, by = 50), lib_column = "KBHarvest", target_column = "RFHarvest", num_samples = 100)
RF_xmap_KB <- ccm(year2, lib, pred, E = 10, lib_sizes = seq(50, 300, by = 50), lib_column = "RFHarvest", target_column = "KBHarvest", num_samples = 100)

#get single rho value for each cross map
KB_xmap_RF1 <- ccm(year2, lib, pred, E = 10, lib_sizes = length(ts), lib_column = "KBHarvest", target_column = "RFHarvest",  random_libs = FALSE)
RF_xmap_KB1 <- ccm(year2, lib, pred, E = 10, lib_sizes = length(ts), lib_column = "RFHarvest", target_column = "KBHarvest",  random_libs = FALSE)
...
##Correlation Analysis (hoping for negative correlation)##
correlation2<-cor(year2$KBHarvest,year2$RFHarvest)
correlation2



#### Time Sereies Analysis (Year3)####
...
# use smap to identify nonlinearity
smap_output <- s_map(ts, lib, pred, E = c(5,9))

# plot s-map output
par(mfrow = c(1,2))
plot(smap_output$theta[smap_output$E == 5], smap_output$rho[smap_output$E == 5], type = "l",xlab = "Nonlinearity (theta)", 
     ylab = "Forecast Skill (rho)")
plot(smap_output$theta[smap_output$E == 9], smap_output$rho[smap_output$E == 9], type = "l",xlab = "Nonlinearity (theta)", 
     ylab = "Forecast Skill (rho)")
...
KB_xmap_RF <- ccm(year3, lib, pred, E = 9, lib_sizes = seq(50, 300, by = 50), lib_column = "KBHarvest", target_column = "RFHarvest", num_samples = 100)
RF_xmap_KB <- ccm(year3, lib, pred, E = 9, lib_sizes = seq(50, 300, by = 50), lib_column = "RFHarvest", target_column = "KBHarvest", num_samples = 100)

#get single rho value for each cross map
KB_xmap_RF1 <- ccm(year3, lib, pred, E = 9, lib_sizes = length(ts), lib_column = "KBHarvest", target_column = "RFHarvest",  random_libs = FALSE)
RF_xmap_KB1 <- ccm(year3, lib, pred, E = 9, lib_sizes = length(ts), lib_column = "RFHarvest", target_column = "KBHarvest",  random_libs = FALSE)
...
##Correlation Analysis (hoping for negative correlation)##
correlation3<-cor(year3$KBHarvest,year3$RFHarvest)
correlation3
